...
def main(fname, NSIDE):
    ...
    for LIMIT in arr:
        ...
        f_name = "ApodizeBinaryMask_%s_%0.1fdeg_apodi.fits" % (key[count],theta_ap)
        hp.fitsfunc.write_map(f_name, imp_map, overwrite=True)
        f_name1 = "MaskedMap_%s_%0.1fdeg_apodi.fits" % (key[count],theta_ap)
        hp.fitsfunc.write_map(f_name1, masked_map, overwrite=True)

        f_name2 ="cl_%s_%0.1fdeg_apodi.fits" % (key[count],theta_ap)
