...
def fit_line(data, fig_num):
    ...
    return out2.params['a'].value, out2.params['b'].value, out2.params['a'].stderr, \
                                            out2.params['b'].stderr, out2.residual 
...
dust_a, dust_b, dust_aerr, dust_berr, dust_residual = fit_line(data1, 1)
...
Sync_a, Sync_b, Sync_aerr, Sync_berr, Sync_residual = fit_line(data2, 2)
...
ax1.plot(data1[:,0], dust_a+data1[:,0]*(-dust_b), '-', color='teal', linewidth=2, 
                                        label='bestfit region-%d'%region_number )

ax1.set_ylim(-1.62, 2.0)
ax1.set_xlim(1.1,)
ax1.set_xlabel(r'$log(\ell)$', fontsize=25, fontstyle='italic', weight='extra bold')
ax1.set_ylabel(r'$log(\ell(\ell+1)C_{\ell}b_{\ell}^{2}W_{pix}/(2\pi))$', fontsize=25, 
                                 fontstyle='italic', weight='extra bold')


ax1.annotate('slope = %0.3f $\pm$ %0.3f'%(dust_b, dust_berr), xy=(1.6, -0.5), xycoords="data",
...
plt.legend(loc=1, fontsize=16, frameon=False, fancybox=False, ncol=1, shadow=False)
#==================================================================================
ax2 = plt.subplot(gs[0, 1])

ax2.plot(data2[:,0], data2[:,1], '-o', color='peru', linewidth=2, label='Spice Sync' )
ax2.plot(data2[:,0], Sync_a+data2[:,0]*(-Sync_b), '-', color='teal', 
              linewidth=2, label='bestfit region-%d'%region_number )

ax2.set_ylim(10,13.8)
ax2.set_xlim(1.1,2.4)

ax2.set_xlabel(r'$log(\ell)$', fontsize=25, fontstyle='italic', weight='extra bold')
ax2.set_ylabel(r'$log(\ell(\ell+1)C_{\ell}b_{\ell}^{2}W_{pix}/(2\pi))$', fontsize=25, 
                                 fontstyle='italic', weight='extra bold')
ax2.annotate('slope = %0.3f $\pm$ %0.3f'%(Sync_b, Sync_berr), xy=(1.6, 10.5), xycoords="data",
                  va="center", ha="center", size=14,
                  bbox=dict(boxstyle="round", fc="w"))
